...
| DB-003 | 建立subscriptions集合，實現subscriptionId、userId、productId、status、cycleType、startDate、nextBillingDate、renewalCount、remainingDiscountPeriods、pendingConversion、extraPeriods等欄位 | DB-001, DB-002 | 已完成 | 4.1 資料表設計 |
| DB-004 | 建立discounts集合，實現discountId、type、value、priority、startDate、endDate、applicableProducts、discountPeriods、extraPeriods等欄位 | 無 | 已完成 | 4.1 資料表設計 |
...
| DDD-019 | 更新Subscription實體以支持extraPeriods（實現applyExtraPeriods、getTotalPeriods、shouldExpire方法，含TDD測試） | DDD-001 | 已完成 | 4.3 核心領域模型設計與方法 |
| DDD-020 | 更新Discount實體以支持extraPeriods（實現hasExtraPeriods、getExtraPeriods方法，含TDD測試） | DDD-006 | 已完成 | 4.3 核心領域模型設計與方法 |
...
| QUEUE-003 | 實現寬限期邏輯（7天），支援手動補款 | QUEUE-001, DDD-004, API-010 | 已完成 | 6.1 訂閱與扣款流程 |
...
| TEST-008 | 添加extraPeriods功能的BDD測試（promo codes提供額外服務期數） | SVC-001, API-014 | 待處理 | 4.3 核心領域模型設計與方法, 6.6 優惠碼應用流程 |
...
| 任務ID | 描述 | 依賴關係 | 狀態 | 設計書 |
|--------|------|----------|------|--------|
| SVC-001 | 更新services以處理extraPeriods邏輯（promoCodeDomainService與billingService） | REP-001, DDD-019, DDD-020 | 進行中 | 4.3 核心領域模型設計與方法, 6.6 優惠碼應用流程 |
- **狀態分布**：
  - 已完成：55個任務
  - 進行中：0個任務
  - 待處理：10個任務
- **額外實現**：
  ...
  - 實現isGracePeriodExpired()和expireGracePeriod()方法
  - 更新系統設計文檔以反映寬限期實現細節
...
- **額外實現**：
  ...
  - 添加aborted狀態處理（當產品不存在時中止訂閱）
  - 更新系統設計文件（添加狀態機圖和產品檢查流程）
  - 實現extraPeriods功能，支持promo codes延長訂閱服務時間而不改變billing amounts
- **建議開發順序**：
  1. 資料庫設置（DB-001~DB-012）
  2. 領域模型與服務（DDD-001~DDD-011）
  3. API實現（API-001~API-019）
...
## 8. 注意事項
- **寬限期邏輯**：實現7天寬限期，支付失敗後訂閱進入grace狀態，gracePeriodEndDate記錄到期時間，GracePeriodCheckerJob每小時檢查並自動取消過期訂閱，手動補款可恢復active狀態。
...
- **額外實現**：續訂優惠邏輯通過DiscountRepository.findRenewalDiscounts動態獲取適用優惠；aborted狀態用於處理產品不存在的邊緣案例；系統設計文件已更新狀態機與流程圖；extraPeriods功能允許promo codes延長訂閱服務時間（例如，12個月 + 3個月額外期數 = 15個月總服務時間），但不改變billing amounts或frequencies，系統繼續自動續訂。