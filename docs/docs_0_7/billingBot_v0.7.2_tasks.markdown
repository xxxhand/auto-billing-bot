...
| 任務ID | 描述 | 依賴關係 | 狀態 | 設計書 |
|--------|------|----------|------|--------|
| DB-001 | 建立users集合，實現userId、tenantId、encryptedData欄位 | 無 | 待處理 | 4.1 資料表設計 |
| DB-002 | 建立products集合，實現productId、name、price、cycleType等欄位 | 無 | 待處理 | 4.1 資料表設計 |
...
| DDD-010 | 實現discountPriorityService領域服務，處理多重優惠優先級（含TDD測試） | DDD-002, DDD-006 | 待處理 | 4.3 核心領域模型設計與方法, 6.2 優惠應用流程 |
...
| API-008 | 實現GET /userPromoCodes，返回用戶可用優惠碼 | DB-005, DDD-007 | 待處理 | 5.1 RESTful API, 5.2 API 資料格式示例 |
| API-009 | 實現POST /payments/retry，手動補款 | DB-006, DDD-008 | 待處理 | 5.1 RESTful API, 6.1 訂閱與扣款流程 |
| API-010 | 實現GET /subscriptions/{id}/history，查詢訂閱與扣款歷史 | DB-008 | 待處理 | 5.1 RESTful API |
| API-011 | 實現GET /admin/logs/export，導出CSV日誌 | DB-008 | 待處理 | 5.1 RESTful API |
| API-012 | 實現JWT認證，包含userId與tenantId | 無 | 待處理 | 5.1 RESTful API |
...
| 任務ID | 描述 | 依賴關係 | 狀態 | 設計書 |
|--------|------|----------|------|--------|
| TEST-001 | 為所有領域模型方法撰寫Jest單元測試 | DDD-001~DDD-010 | 待處理 | 4.3 核心領域模型設計與方法 |
| TEST-002 | 為所有API端點撰寫Postman測試 | API-001~API-012 | 待處理 | 5.1 RESTful API |
...
| DOC-001 | 撰寫Markdown格式API文件，遵循OpenAPI 3.0 | API-001~API-012 | 待處理 | 5.1 RESTful API, 5.2 API 資料格式示例 |
...
## 8. 注意事項
- **TDD實踐**：每個領域模型、API及支付mock任務需先撰寫測試，涵蓋正常、邊緣與異常案例（如大小月、閏年、優惠無效等）。
- **DDD原則**：確保業務邏輯封裝在領域層，避免應用層直接操作資料庫。Subscription為核心聚合根，控制所有訂閱相關操作。
