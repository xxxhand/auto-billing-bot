...
- **描述：** 
   **系統原則：僅套用單一優惠**。無論優惠類型為何，系統始終依優先級選擇並套用最高優先級的單一優惠，不支援多重優惠疊加。
   ...
   5. **支援「額外服務期數」**：優惠可以提供額外的服務時間，而非僅僅是價格折扣。訂閱總期數 = 原始期數 + 額外期數，此為參考資訊，系統仍會繼續自動續訂。下次扣款日將延後至原始期數 + 額外期數後。
- **備註：** 例如：固定折扣100元，或打七折 (30% off)。當優惠綁定特定產品時，只能在訂閱該產品時套用。額外服務期數適用於年付產品轉換為15個月服務等場景。
...
- **描述：** 優惠碼可指定優惠價格及優惠期數。分為：
  ...
  4. **支援「額外服務期數」**：優惠碼可以提供額外的服務時間，延長訂閱總期數。
...
- **情境二：獨立優惠碼（專屬代碼、一次性、無門檻）**
  ...
  - 後台可查詢各代碼的使用者與使用狀態。
- **情境三：額外服務期數優惠碼**
  - 系統管理員建立優惠碼，提供額外的服務時間（如年付產品 + 優惠碼 = 15個月服務）。
  - 支援設定額外期數（例如：3個月），訂閱總期數 = 原始期數 + 額外期數。
  - 下次扣款日將延後至原始期數 + 額外期數後，系統仍會繼續自動續訂，此為參考資訊。
  - 可與價格折扣結合使用。
...
| 4.1 優惠與方案（額外服務期數） | 新用戶 | 可以使用優惠碼獲得額外的服務時間 | 我能獲得更長的服務期限，提升使用價值 |
...
**And** 使用者在訂閱期間可以使用優惠碼，但下次扣款才生效  
**And** 系統提供優惠碼機制：  
  **-** 優惠碼可設定折扣金額、百分比折扣或固定結帳金額  
  **-** 優惠碼可設定使用次數限制（一碼多人或一碼一人）  
  **-** 優惠碼可綁定特定產品或全域適用  

**Feature:** 用戶訂閱產品  
**As a** 新用戶  
**I want to** 使用固定結帳金額優惠碼訂閱年付產品  
**So that** 我可以獲得特殊的固定價格優惠  

**Scenario:** 成功訂閱年付產品（使用固定結帳金額優惠碼）  
**Given** 我是一個新用戶，尚未有任何訂閱  
**And** 系統中有可訂閱的年付產品（價格：每年 $2490）  
...
**When** 我再輸入優惠碼 "FIXED1500"  
**Then** 系統應該驗證優惠碼有效性  
**And** 顯示最終價格應該是 $1500（固定結帳金額覆蓋之前的折扣）  
**And** 顯示優惠碼折扣明細（優惠碼：FIXED1500，類型：固定結帳金額，金額：$1500）  
**And** 系統應該提示只能使用一個優惠碼

### 情境5：續訂月付產品第二次扣款(無優惠碼) [完成]
...
**Feature:** 訂閱續訂  
**As a** 現有訂閱用戶  
**I want to** 系統自動續訂月付產品  
**So that** 服務不中斷  

**Scenario:** 第二次扣款成功（無優惠碼）  
...
**Feature:** 訂閱續訂  
**As a** 現有訂閱用戶  
**I want to** 系統自動續訂年付產品  
**So that** 服務不中斷  

**Scenario:** 第二次扣款成功（年付產品續訂，無優惠碼）  
...
**And** 使用者在訂閱期間可以使用優惠碼，但下次扣款才生效  
**And** 系統提供週年慶檔期優惠：  
  **-** 年付產品續訂期間提供固定折扣 $2000（從原價 $2490 折扣至 $490）  
  **-** 週年慶優惠優先級高於續訂折扣  
  **-** 週年慶優惠適用期間：2025/10/1 至 2025/10/31  

**Feature:** 訂閱續訂  
**As a** 現有訂閱用戶  
**I want to** 系統自動續訂年付產品並套用週年慶優惠  
**So that** 我能以優惠價格繼續使用服務  

**Scenario:** 第二次扣款成功（年付產品續訂，週年慶折扣$2000）  
**Given** 我已經訂閱年付產品（價格：每年 $2490）  
**And** 訂閱狀態為 "active"  
**And** 續訂次數為 0  
**And** 已經進行過第一次扣款（扣款 $1000 成功，第一年優惠）  
...
### 情境8：續訂年付產品第二次扣款(週年慶折扣2000，專屬優惠碼「固定結帳金額」300)
...
**Feature:** 訂閱續訂  
**As a** 現有訂閱用戶  
**I want to** 系統自動續訂年付產品並套用優惠碼與週年慶優惠  
**So that** 我能獲得最佳優惠價格繼續使用服務  

**Scenario:** 第二次扣款成功（年付產品續訂，優惠碼固定結帳金額$300覆蓋週年慶折扣$2000）  
**Given** 我已經訂閱年付產品（價格：每年 $2490）  
**And** 訂閱狀態為 "active"  
**And** 續訂次數為 0  
**And** 已經進行過第一次扣款（扣款 $1000 成功，第一年優惠）  
**And** 下次扣款日期已到（從訂閱開始日後 1 年）  
**And** 目前是週年慶期間（2025/10/15）  
**And** 我在訂閱期間已使用優惠碼 "ANNIV300"（固定結帳金額 $300，適用於年付產品，一碼一人使用，優惠期數 3）  
**When** 系統進行第二次扣款  
...
**And** 下次扣款日期應該更新為從現在開始的 1 年後  
**And** 訂閱狀態應該保持 "active"  
**And** 訂閱的 appliedDiscountId 應該是 'promo-fixed-300'  
**And** 訂閱的 remainingDiscountPeriods 應該從 3 減少到 2  
**And** 系統應該記錄續訂成功的事件及套用的優惠碼優惠

### 情境9：新用戶使用「額外服務期數」優惠碼訂閱年付產品
...
**And** 使用者在訂閱期間可以使用優惠碼，但下次扣款才生效  
**And** 系統提供優惠碼機制：  
  **-** 優惠碼可設定折扣金額、百分比折扣、固定結帳金額或額外服務期數  
  **-** 優惠碼可設定使用次數限制（一碼多人或一碼一人）  
  **-** 優惠碼可綁定特定產品或全域適用  

**Feature:** 用戶訂閱產品  
**As a** 新用戶  
**I want to** 使用額外服務期數優惠碼訂閱年付產品  
**So that** 我可以獲得更長的服務時間  

**Scenario:** 成功訂閱年付產品（使用額外服務期數優惠碼）  
**Given** 我是一個新用戶，尚未有任何訂閱  
**And** 系統中有可訂閱的年付產品（價格：每年 $2490，原始服務期數：12個月）  
**And** 系統中有有效的優惠碼 "EXTRA3MONTHS"（提供額外3個月服務期數，適用於年付產品，一碼一人使用，尚未被使用）  
**When** 我查詢可訂閱產品列表  
**Then** 我應該看到年付產品的完整資訊（產品名稱、價格 $1000、年付週期）  
**And** 顯示價格應該是第一次訂閱折扣價 $1000（系統自動折扣）  

**When** 我輸入優惠碼 "EXTRA3MONTHS"  
**Then** 系統應該驗證優惠碼有效性  
**And** 顯示最終價格應該是 $1000（系統自動折扣）  
**And** 顯示優惠碼優惠明細（優惠碼：EXTRA3MONTHS，類型：額外服務期數，額外期數：3個月）  
**And** 顯示總服務期數：15個月（原始12個月 + 額外3個月）  

**When** 我選擇年付產品並確認訂閱  
**Then** 系統應該為我創建新的訂閱記錄  
**And** 訂閱狀態應該是 "active"  
**And** 訂閱記錄應該包含優惠碼使用資訊（優惠碼：EXTRA3MONTHS，使用時間：當前時間）  
**And** 訂閱的 extraPeriods 應該是 3  
**And** 下次扣款日期應該是從今天開始的 15 個月後（原始12個月 + 額外3個月）  
**And** 續訂次數應該是 0  
**And** 我應該收到訂閱成功的確認訊息（包含總服務期數：15個月）  
...
**When** 系統進行第二次扣款（第15個月後）  
**Then** 應該從我的支付方式扣款 $1990（續訂價格）  
**And** 扣款記錄應該被正確保存  
**And** 訂閱的續訂次數應該更新為 1  
**And** 下次扣款日期應該更新為從現在開始的 1 年後  
**And** 訂閱狀態應該保持 "active"  
