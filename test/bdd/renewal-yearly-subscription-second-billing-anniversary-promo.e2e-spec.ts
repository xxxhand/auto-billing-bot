...
describe('BDD: 續訂年付產品第二次扣款(週年慶折扣2000，專屬優惠碼「固定結帳金額」300)', () => {
  let agent: superTest.SuperAgentTest;
  const dbHelper = new MongoHelper('bdd_renewal_yearly_second_billing_anniversary_promo');
  ...
  // 優惠碼 - 固定結帳金額$300，最高優先級
  const mockPromoCode: IPromoCodeDocument = {
    _id: dbHelper.newObjectId(),
    code: 'ANNIV300',
    discountId: 'promo-fixed-300',
    usageLimit: 1, // 一碼一人使用
    isSingleUse: true,
    usedCount: 0,
    minimumAmount: 0, // 無最低消費門檻
    applicableProducts: ['yearly-product-001'], // 只適用於yearly產品
    valid: true,
  };

  const mockPromoCodeDiscount: IDiscountDocument = {
    _id: dbHelper.newObjectId(),
    discountId: 'promo-fixed-300',
    type: 'fixed_price', // 固定結帳金額
    value: 300, // 固定結帳金額$300
    priority: 30, // 最高優先級，覆蓋所有其他優惠
    startDate: new Date(2020, 0, 1), // 長期有效
    endDate: new Date(2030, 11, 31),
    applicableProducts: ['yearly-product-001'], // 只適用於yearly產品
    discountPeriods: 3,
  ...
  beforeAll(async () => {
    ...
    await Promise.all([
      ...
      db.getCollection(discountCol).insertOne(mockPromoCodeDiscount),
  ...
  describe('Feature: 訂閱續訂', () => {
    describe('As a 現有訂閱用戶', () => {
      describe('I want to 系統自動續訂年付產品並套用優惠碼與週年慶優惠', () => {
        describe('So that 我能獲得最佳優惠價格繼續使用服務', () => {

          describe('Scenario: 第二次扣款成功（年付產品續訂，優惠碼固定結帳金額$300覆蓋週年慶折扣$2000）', () => {
            ...
            // And 我在訂閱期間已使用優惠碼 "ANNIV300"（固定結帳金額 $300，適用於年付產品，一碼一人使用）
            it('Given: 用戶已有年付訂閱且第一次扣款成功，下次扣款日期已到，目前在週年慶期間，已使用優惠碼', async () => {
              // 創建現有訂閱記錄
              ...
              // 通過API應用優惠碼到訂閱（在訂閱期間使用，只在下次扣款生效）
              const applyPromoResponse = await agent
                .post(`${process.env.DEFAULT_API_ROUTER_PREFIX}/v1/promoCodes/applyPromo`)
                .send({
                  userId: mockUser.userId.toHexString(),
                  promoCode: 'ANNIV300',
                  orderAmount: 2490, // 原價
                  productIds: [mockYearlyProduct.productId],
                  applyToSubscription: true,
                  subscriptionId: existingSubscription.subscriptionId,
                });

              expect(applyPromoResponse.status).toBe(201);
              expect(applyPromoResponse.body.code).toBe(0); // API成功響應
              expect(applyPromoResponse.body.result.appliedToSubscription).toBe(true);
              expect(applyPromoResponse.body.result.remainingDiscountPeriods).toBe(3); // API目前硬編碼3個週期

              // 驗證訂閱已被應用優惠碼折扣
              const updatedSubscription = await db.getCollection('Subscriptions').findOne({
                subscriptionId: existingSubscription.subscriptionId
              });
              expect(updatedSubscription.appliedDiscountId).toBe('promo-fixed-300');
            ...
            it('When: 系統進行第二次扣款', async () => {
              ...
              jest.spyOn(mockPaymentGateway, 'charge').mockResolvedValue({
                success: true,
                transactionId: 'txn-yearly-second-anniv-promo-12345',
              });
              ...
              expect(billingResult.transactionId).toBe('txn-yearly-second-anniv-promo-12345');
            });

            // Then 應該從我的支付方式扣款 $300（優惠碼固定結帳金額覆蓋週年慶折扣 $2000）
            // And 扣款記錄應該被正確保存
            // And 訂閱的續訂次數應該更新為 1
            // And 下次扣款日期應該更新為從現在開始的 1 年後
            // And 訂閱狀態應該保持 "active"
            // And 系統應該記錄續訂成功的事件及套用的優惠碼優惠
            it('Then: 驗證第二次扣款成功並套用優惠碼固定結帳金額$300，覆蓋週年慶折扣', async () => {
              // 驗證扣款金額為300（優惠碼固定結帳金額覆蓋週年慶折扣2000）
              ...
              // 找到第二次扣款記錄
              const secondPayment = paymentAttempts.find((p: any) => p.status === 'success' && p.amount === 300 && p.attemptId !== paymentAttempts[0].attemptId);
              ...
              // 驗證優惠碼折扣已被應用並更新週期
              expect(updatedSubscription.appliedDiscountId).toBe('promo-fixed-300'); // 優惠碼折扣ID保持
              expect(updatedSubscription.remainingDiscountPeriods).toBe(2); // 從3減為2（已使用1次）
              ...
              // 驗證優惠碼使用次數已更新（已使用1次，剩餘0次）
              const updatedPromoCode = await db.getCollection(promoCodeCol).findOne({
                code: 'ANNIV300'
              });
              expect(updatedPromoCode.usedCount).toBe(1);
              expect(updatedPromoCode.usageLimit - updatedPromoCode.usedCount).toBe(0); // 剩餘0次
            });
          });
        });
      });
