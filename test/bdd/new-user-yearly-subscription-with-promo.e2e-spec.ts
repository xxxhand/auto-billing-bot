...
describe('BDD: 新用戶訂閱年付產品（有優惠碼）', () => {
  let agent: superTest.SuperAgentTest;
  const dbHelper = new MongoHelper('bdd_new_user_yearly_subscription_with_promo');
  ...
  // First-time yearly subscription discount rule (lower priority)
  ...
  const promoCodeDiscountRule = {
    ...
    conditions: {
      'product.cycleType': 'yearly',
      'subscription.isFirstTimeSubscription': true,
      'promoCode.code': 'NEWYEAR2024',
      'currentDate': { operator: 'lte', value: '2026-12-31' }
    },
    actions: {
      discount: {
        type: 'fixed',
        value: 500 // 2490 - 500 = 1990 discount
      }
    },
  ...
  // Promo code discount entity for testing
  const mockPromoDiscount = {
    _id: dbHelper.newObjectId(),
      ...
      describe('I want to 訂閱年付產品', () => {
        describe('So that 我可以開始使用服務', () => {
          ...
          describe('Scenario: 成功訂閱年付產品（有優惠碼）', () => {
            ...
            // When 我查詢可訂閱產品列表並提供優惠碼
            it('When: 查詢可訂閱產品列表並提供優惠碼', async () => {
            ...
            // Then 我應該看到年付產品的完整資訊（產品名稱、價格 $1990、年付週期）
            // And 顯示價格應該是優惠碼折扣價 $1990（優惠碼優先於第一次訂閱折扣）
            it('Then: 看到年付產品完整資訊且有優惠碼折扣', () => {
              const yearlyProduct = productsResponse.find((p: any) => p.productId === mockYearlyProduct.productId);
              expect(yearlyProduct.name).toBe('年付產品');
              expect(yearlyProduct.originalPrice).toBe(2490);
              expect(yearlyProduct.cycleType).toBe('yearly');
              // 優惠碼折扣優先，顯示 $1990 (2490 - 500)
              expect(yearlyProduct.discountedPrice).toBe(1990);
            });

            // When 我選擇年付產品並確認訂閱（使用優惠碼）
            it('When: 選擇年付產品並確認訂閱（使用優惠碼）', async () => {
            ...
            it('Then: 創建新的訂閱記錄並驗證訂閱狀態', async () => {
              ...
              expect(dbSubscription.promoCode).toBe('NEWYEAR2024');
            });
            ...
            it('Then: 驗證扣款成功並保持續訂次數為 0', async () => {
              ...
              expect(successfulPayment).toBeTruthy();
              expect(successfulPayment.amount).toBe(1990); // 驗證扣款金額（優惠碼折扣價）
            });
          });
        });
      });
